...
sortedAssignment = assignment(:,2:3);
sortedAssignment = sort(sortedAssignment,2);
for i = 1 : length(assignment)    
    t2= sortedAssignment(i,:);
    % first we need find index from genotypesToAlleles given assignment
    % value
    [dummy,index] = ismember(t2,genotypesToAlleles,'rows');
    if index>0
        % this means genotype combination same with out assignment
        if assignment(i,1) == 1 % this is visiable trait
            phenotypeFactor.val(i) = alphaList(index);
        else
            phenotypeFactor.val(i) = 1 - alphaList(index);
        end
    else 
        t3 = 'assigment is not in genotype combination'
    end
end
