...
% there James has the parents are Ira and Robin 
...
%   physical trait ( here hre probability is the prob of Non Mendelian Factor)
%
% Output:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
for i = 1 : 2*numPeople
    if i <= numPeople % vaariable for the ith people
        if pedigree.parents(i)  == [0,0] % if they don't have parents. 
            factorList(i) = genotypeGivenAlleleFreqsFactor(alleleFreqs,i);
        else
            %tmpPar = pedigree.parents(i,:);
            %factorList(i).var = [i pedigree.parents(i,:)];
            %factorList(i).card = linspace(numPeople,numPeople,numPeople);
